...
public sealed class TelemetryDbConnectionTests
{
    ...
    private static MeterListener CreateMeterListener(List<double> measurements)
    {
        ...
        ml.SetMeasurementEventCallback<double>((_, measurement, _, _) =>
    ...
    public async Task EmitMetricsDurationMatchesActualTimeAsync(double delayMs)
    {
        ...
        await using (mock.ConfigureAwait(false))
        {
            var conn = CreateConnection(mock, new TelemetryDbConnectionOptions { EmitTraces = false, EmitMetrics = true });
            await using (conn)
            {
                var sw = System.Diagnostics.Stopwatch.StartNew();
    ...
    public static TheoryData<string, Func<TelemetryDbConnection, Task>> AsyncOperations =>
        new()
        {
            ...
            {
                "change_database",
                async c => await c.ChangeDatabaseAsync("MockDb").ConfigureAwait(false)
            },
            {
                "get_schema",
                async c => await c.GetSchemaAsync().ConfigureAwait(false)
            },
            {
                "begin_transaction",
                async c =>
    ...
    public async Task AllAsyncOperationsEmitTracesCreateActivity(string expectedOperation, Func<TelemetryDbConnection, Task> runOperation)
    {
        ...
        await using var mock = CreateMockDbConnection();
        await using var conn = CreateConnection(mock, new TelemetryDbConnectionOptions { EmitTraces = true, EmitMetrics = false });
    ...
    public void EnrichActivityIsCalledWithCommand()
    {
        ...
        using var conn = CreateConnection(mock, new TelemetryDbConnectionOptions
        {
            EmitTraces = true,
            EmitMetrics = false,
            EnrichActivity = (_, cmd) => capturedCommand = cmd,
        });
    ...
    public void EnrichMetricsIsCalledWithCommand()
    {
        ...
        using var conn = CreateConnection(mock, new TelemetryDbConnectionOptions
        {
            EmitTraces = false,
            EmitMetrics = true,
            EnrichMetrics = (_, _) => enrichMetricsCalled = true,
        });
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT 1";
        cmd.ExecuteNonQuery();
    ...
    public void EnrichActivityExceptionInCallbackOperationStillSucceeds()
    {
        ...
        using var conn = CreateConnection(mock, new TelemetryDbConnectionOptions
        {
            EmitTraces = true,
            EmitMetrics = false,
            EnrichActivity = (_, _) => throw new InvalidOperationException("test"),
        });
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT 1";
    ...
    public void EnrichMetricsExceptionInCallbackOperationStillSucceeds()
    {
        ...
        using var conn = CreateConnection(mock, new TelemetryDbConnectionOptions
        {
            EmitTraces = false,
            EmitMetrics = true,
            EnrichMetrics = (_, _) => throw new InvalidOperationException("test"),
        });
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT 1";
        var exception = Record.Exception(() => cmd.ExecuteNonQuery());
