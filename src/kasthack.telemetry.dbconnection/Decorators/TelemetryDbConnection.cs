...
using kasthack.telemetry.dbconnection.Constants;
...
public sealed class TelemetryDbConnection : DbConnection
{
    ...
    /// <inheritdoc/>
    public override Task OpenAsync(CancellationToken cancellationToken) => _options.TrackConnectionManagement.HasFlag(ConnectionManagementTracking.Open)
            ? ExecuteInstrumentedAsync(ConnectOperation, () => _inner.OpenAsync(cancellationToken))
    ...
    /// <inheritdoc/>
    public override DataTable GetSchema() => ExecuteInstrumented<DataTable>(GetSchemaOperation, _inner.GetSchema);

    /// <inheritdoc/>
    public override DataTable GetSchema(string collectionName) => ExecuteInstrumented<DataTable>(GetSchemaOperation, () => _inner.GetSchema(collectionName));

    /// <inheritdoc/>
    public override DataTable GetSchema(string collectionName, string?[] restrictionValues) => ExecuteInstrumented<DataTable>(GetSchemaOperation, () => _inner.GetSchema(collectionName, restrictionValues));

#if NET7_0_OR_GREATER
    /// <inheritdoc/>
    public override Task<DataTable> GetSchemaAsync(CancellationToken cancellationToken = default) => ExecuteInstrumentedAsync<DataTable>(GetSchemaOperation, () => _inner.GetSchemaAsync(cancellationToken));

    /// <inheritdoc/>
    public override Task<DataTable> GetSchemaAsync(string collectionName, CancellationToken cancellationToken = default) => ExecuteInstrumentedAsync<DataTable>(GetSchemaOperation, () => _inner.GetSchemaAsync(collectionName, cancellationToken));

    /// <inheritdoc/>
    public override Task<DataTable> GetSchemaAsync(string collectionName, string?[] restrictionValues, CancellationToken cancellationToken = default) => ExecuteInstrumentedAsync<DataTable>(GetSchemaOperation, () => _inner.GetSchemaAsync(collectionName, restrictionValues, cancellationToken));
#endif
    ...
    public override ISite? Site { get => _inner.Site; set => _inner.Site = value; }
    #endregion
    ...
    internal Activity? StartActivity(string operationName, string? dbStatement, DbCommand? command = null)
    {
        ...
        if (dbStatement is not null && ShouldCaptureStatement(command))
        {
...
#else
        var duration = (double)(Stopwatch.GetTimestamp() - startTimestamp) / Stopwatch.Frequency;
#endif
...
#pragma warning restore CA1031
    ...
    private bool ShouldCaptureStatement(DbCommand? command) =>
        _options.CaptureStatements.HasFlag(
            command?.CommandType == System.Data.CommandType.StoredProcedure
                ? CaptureStatements.StoredProcedures
                : CaptureStatements.Text);
